module Products
  class ExportAsCsvJob < ApplicationJob
    queue_as :csv_export
    ...
    def perform(internal_identifier)
      ...
      records = Spree::ProductCsv.custom_ransack(query).result
    ...
    def generate_csv_for(records)
      begin
        ...
        CSV.generate do |csv|
          csv << Spree::ProductCsv.headers
          records.each do |record|
            csv << Spree::ProductCsv.body.map { |field| record.send(field) }
            if (counter % 10).zero?
