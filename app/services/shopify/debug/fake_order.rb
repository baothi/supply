# The purpose of this class is to help us create fake Retailer Orders
# with items that are linked (i.e. live products)
module Shopify
  module Debug
    class FakeOrder
      ...
      #   # end
      #   # selected_variant_listings.compact
      #   # raise 'Empty variants' if selected_variant_listings.empty?
      #   # puts "Added #{selected_variant_listings.count} elements to work with".yellow
      #   # selected_variant_listings
      #   selected_variants = []
      #   while selected_variants.count < @num_of_line_items.to_i
      #     random_variant = @supplier.variants.sample
      #     selected_variants << random_variant_listing unless
      #         selected_variants.include?(random_variant)
      #   end
      #   selected_variants
      # end
      ...
      def assign_local_variants
        ...
        select_random_variants = [1]
        selected_variants = select_random_variants

        selected_variants.each do |variant|
          shopify_variant = get_shopify_variant(variant)
      ...
      def get_shopify_variant(_variant)
        begin
          ShopifyAPI::Variant.last
        rescue => ex
