class Supplier::IntegrationsController < Supplier::BaseController
  ...
  def shopify_login
    ...
    redirect_url = "http://#{host}/supplier/integrations/shopify/login/callback"
  ...
  def shopify_login_callback
    unless params[:code]
      return redirect_to supplier_integrations_shopify_path, error: 'Could not Authenticate'
    end
  ...
  def shopify_credential_params
    params.require(:shopify_credential).permit(
      :api_key, :password, :shared_secret, :supplier_id, :store_url
    )
  end
  ...
  def set_shopify_session
    ...
    @shopify_credential = Spree::ShopifyCredential.find_or_initialize_by(supplier_id: @team.id)
...
# 'hingeto-development-store.myshopify.com' ||
