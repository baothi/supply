class Retailer::TeamController < Retailer::BaseController
  ...
  before_action :ensure_current_user_is_retailer_owner, only: :delete_account
  ...
  def add_account
    if current_retailer.add_team_member(
      user_params.merge(shopify_url: current_spree_user.shopify_url),
      params[:spree_role_id]
    )
      return redirect_to retailer_team_index_path, notice: 'User added successfully'
    end

    redirect_to retailer_team_index_path, alert: 'Error creating new account'
  end

  def delete_account
    if @team_member && @team_member&.role&.name == Spree::Retailer::RETAILER_OWNER
      flash[:alert] = 'Retailer Owner account cannot be removed.'
      redirect_to retailer_team_index_path and return
    end
  ...
  def ensure_supplier_admin_role
    ...
    return if role && role.name == Spree::Retailer::RETAILER_ADMIN

    flash[:alert] = 'Invalid role id supplied'
    redirect_back fallback_location: member_retailer_team_index_path
  end
  ...
  def ensure_user_not_exist
    ...
    redirect_to retailer_team_index_path, alert: 'Email already in use within team'
  end

  def find_team_member_from_current_team
    identifier = params[:internal_identifier] || team_member_params[:internal_identifier]
    @team_member = current_retailer.team_members.find_by(internal_identifier: identifier)
    return if @team_member

    redirect_to retailer_team_index_path, alert: 'Team member not found'
  end
